...
public class AiExecuteRequest {
    ...
    // ===== 业务输入（v11.52 语义分层） =====
    @Size(max = 8000)
    private String userInput;          // 用户自由文本（人打的原始输入）：对话/检测/生成类场景
                                      // 的主要原料；网关意图判断与语义缓存键统一消费
    private Map<String, Object> input; // 结构化业务参数（userId/range/position 等强类型键值），
                                      // 由各场景 Handler 自定义取值，网关不解释
...
> **入参语义分层（v11.52）**：`userInput`（顶层字段）承载"用户说了什么"——网关横切关注点
> （IntentClassifier 意图路由、SemanticCache 键计算）只读它；`input` 承载"业务上下文是什么"——
> 结构化强类型参数，网关透传不解释。chat 收口进网关后，对话消息天然映射 userInput，
> 意图判断分支即成为场景路由器。
...
> **实现演进注记（v11.48 / v11.49）**：
>
> 1. **`execute(request, config)` 双参方法**（v11.48）：网关执行时直接下发场景配置，Handler 无需静态 ThreadLocal 取配置。
>    上述单参 `execute(request)` 保留兼容但已不推荐，存量实现应迁移至双参方法。
> 2. **Agent 人设合并**（v11.49）：`ai_scene_config.agent_id` 绑定智能体时，网关在缓存键计算**之前**读取
>    `ai_agent.system_prompt`，以 `request.input` 渲染 `{{占位符}}` 后注入 `input.agentPersona`；
>    Handler 在 `AbstractAiSceneHandler#mergePersona` 中将其**前置**到系统提示词。
>    分工：**Agent 定义"是谁"（人设/语气/知识库），场景配置定义"做什么/怎么输出"（任务模板/输出结构）**。
>    Agent 禁用/无提示词/加载失败均静默跳过（按无人设执行）；人设变更自动失效语义缓存。
> 3. **配置即时生效**（v11.48）：`AiSceneRegistry#getConfig` 直查 `ai_scene_config`（priority DESC LIMIT 1），
>    管理端改提示词模板/输出结构/Agent 绑定后**下次调用立即生效**，无需重启。
