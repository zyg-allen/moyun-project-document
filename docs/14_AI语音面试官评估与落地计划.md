...
| 文本模拟面试（快练版） | 85% | [MockInterviewServiceImpl.java](file:///d:/zyg_new_work/moyun-project-document/moyun-server/src/main/java/com/moyun/ext/cms/service/impl/MockInterviewServiceImpl.java) + [PortalMockInterviewController.java](file:///d:/zyg_new_work/moyun-project-document/moyun-server/src/main/java/com/moyun/portal/controller/PortalMockInterviewController.java) + [MockInterviewPage.vue](file:///d:/zyg_new_work/moyun-project-document/moyun-portal/src/pages/interview/MockInterviewPage.vue) | start→answer→finish 三阶段；规则评分（关键词×80 + 长度×20）；AI反馈可切换（`isEnabled()` 目前 false）；三路抽题（薄弱点+必备技能+随机）；画像快照已落库 |
...
| 错题本/答题历史/收藏 | 80% | `MyBookmarksPage.vue` / `MyAttemptsPage.vue` + 对应 API | 可独立运行；薄弱点画像通过 `updateMockInterviewStats` / `refreshWeakTags` 回写（面试结束时同步触发） |
| OJ 代码判题 | 70% | `QuestionDetailPage.vue` 代码编辑器 + `/portal/judge/submit` 同步返回 | 非本方案主线 |
| **用户画像/薄弱点** | **75%** | `UserProfileSnapshotServiceImpl.buildSnapshot()` + `refreshWeakTags()` | 三路召回已实现（薄弱点×最多3 + 必备技能×最多2 + 随机兜底），且已在 mockInterview 内运行；但 "基于简历"（40%配比）与"基于岗位必备"（30%配比）的精确权重尚未对齐方案 §3.1 |
...
| **字典体系** | **95%** | v9.6 已补齐 27 类业务字典，面试域全覆盖：`portal_question_difficulty/type/mock_scene/portal_mock_scene`；`useDictData` composable 可直接复用 | 方案 §11.4 style/scene/question_source 字典化，只需追加 3~4 个字典类型，不触碰代码 |
