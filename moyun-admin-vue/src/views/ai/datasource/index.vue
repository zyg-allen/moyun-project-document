...
const testConnection = async () => {
  testing.value = true
  try {
    const res = await request({ url: '/cms/ai/data-analysis/datasources/test', method: 'post', data: formData})
...
const saveDataSource = async () => {
  ...
  try {
    if (formData.id) {
      await request({ url: `/cms/ai/data-analysis/datasources/${formData.id}`, method: 'put', data: formData})
      ElMessage.success('更新成功')
    } else {
      await request({ url: '/cms/ai/data-analysis/datasources', method: 'post', data: formData})
...
const deleteDataSource = (row) => {
  ElMessageBox.confirm('确定要删除该数据源吗?', '提示', {
    ...
    try {
      await request({ url: `/cms/ai/data-analysis/datasources/${row.id}`, method: 'delete'})
...
const toggleEnabled = async (row) => {
  try {
    await request({ url: `/cms/ai/data-analysis/datasources/${row.id}`, method: 'put', data: row})
...
const checkHealth = async (row) => {
  try {
    const res = await request({ url: `/cms/ai/data-analysis/datasources/${row.id}/health`, method: 'get' })
    console.log('健康检查完整响应:', res)
    console.log('响应数据:', res.data)
    
    // 更新健康状态 - 后端返回格式：{success: true, message: "healthy"}
    // request 拦截器返回响应体本身，res.msg 即状态值
    const status = res.msg || res.data
...
const syncMetadata = async (row) => {
  try {
    await request({ url: `/cms/ai/data-analysis/datasources/${row.id}/sync`, method: 'post'})
...
const viewTables = async (row) => {
  ...
  try {
    const res = await request({ url: `/cms/ai/data-analysis/datasources/${row.id}/tables/info`, method: 'get' })
...
const viewColumns = async (tableName) => {
  ...
  try {
    const res = await request({ url: `/cms/ai/data-analysis/datasources/${currentDatasource.value.id}/tables/${tableName}/schema`, method: 'get' })
...
<style scoped src="@/views/ai/styles/datasource-manage.css"></style>
