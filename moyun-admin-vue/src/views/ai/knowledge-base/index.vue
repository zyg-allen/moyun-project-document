...
const submitUpload = async () => {
  ...
  try {
    const response = await request({ url: '/cms/ai/knowledge-base/upload', method: 'post', data: formData, headers: {
...
const startPolling = (knowledgeId) => {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      // 先查询实时进度
      const progressResponse = await request({ url: `/cms/ai/knowledge/progress/${knowledgeId}`, method: 'get' })
      ...
      // 再查询知识库状态
      const statusResponse = await request({ url: `/cms/ai/knowledge-base/${knowledgeId}`, method: 'get' })
      const knowledge = statusResponse.data
...
const openConfigForKnowledge = async (knowledge) => {
  try {
    // 获取推荐模板
    const response = await request({ url: `/cms/ai/knowledge-base/templates/recommended?fileType=${knowledge.fileType}`, method: 'get' })
...
const startProcessing = async (knowledgeId) => {
  try {
    const response = await request({ url: `/cms/ai/knowledge-base/start-processing/${knowledgeId}`, method: 'post'})
...
const deleteKnowledge = async (id) => {
  try {
    ...
    const response = await request({ url: `/cms/ai/knowledge-base/${id}`, method: 'delete'})
...
const reprocess = async (knowledge) => {
  try {
    ...
    if (knowledge.processingStatus === 'failed' && knowledge.configCompleted) {
      ...
      const response = await request({ url: `/cms/ai/knowledge-base/reprocess/${knowledge.id}`, method: 'post'})
...
const loadSegments = async (knowledgeBaseId) => {
  ...
  try {
    const response = await request({ url: `/cms/ai/knowledge-base/${knowledgeBaseId}/segments`, method: 'get' })
    // 后端返回的是 ListResponse 格式：{ list: [], total: n }
    segments.value = response.data?.list || []
  } catch (error) {
...
const saveEdit = async () => {
  ...
  try {
    const response = await request({ url: `/cms/ai/knowledge-base/${editForm.value.id}/metadata`, method: 'put', data: {
...
const loadStats = async () => {
  ...
  try {
    const response = await request({ url: '/cms/ai/knowledge-base/stats', method: 'get' })
...
const executeRetrievalTest = async () => {
  ...
  try {
    const response = await request({ url: `/cms/ai/knowledge-base/${currentTestKnowledge.value.id}/test-retrieval`, method: 'post', data: retrievalTestForm.value})
...
const downloadFile = async () => {
  if (!currentPreviewKnowledge.value) return
  try {
    // 走 request 拦截器携带 Authorization，后端 download 接口需鉴权
    const blob = await request({
      url: `/cms/ai/knowledge-base/${currentPreviewKnowledge.value.id}/download`,
      method: 'get',
      responseType: 'blob'
    })
    ...
    a.href = url
    a.download = currentPreviewKnowledge.value.fileName || 'download'
...
<style scoped src="@/views/ai/styles/knowledge-manage.css"></style>
