    ...
    <el-dialog v-model="dialog.visible" :title="dialog.form.id ? '修改套餐' : '新增套餐'" width="520px" append-to-body>
      <el-form :model="dialog.form" label-width="90px">
        ...
        <el-form-item label="权益说明">
          <el-input v-model="dialog.form.description" type="textarea" :rows="2" maxlength="255" placeholder="如：语音面试不限场次、深度面试报告、题库全量访问" />
        </el-form-item>
...
<script setup name="InterviewVipPackage">
import { listInterviewVipPackage, addInterviewVipPackage, updateInterviewVipPackage, delInterviewVipPackage } from "@/api/cms/interviewVip";
...
function openDialog(row) {
  dialog.form = row ? { ...row } : { name: "", price: 19.9, originalPrice: null, durationDays: 30, description: "", popular: false, sort: 0 };
  dialog.visible = true;
}
...
function saveDialog() {
  ...
  dialog.saving = true;
  const req = form.id ? updateInterviewVipPackage(form) : addInterviewVipPackage(form);
  req.then(() => {
...
function toggleStatus(row) {
  updateInterviewVipPackage({ id: row.id, status: row.status }).then(() => {
...
function handleDelete(row) {
  proxy.$modal.confirm('确认删除套餐「' + row.name + '」？（已有订阅订单的套餐仅可下架）').then(() => {
    return delInterviewVipPackage(row.id);
  }).then(() => {
