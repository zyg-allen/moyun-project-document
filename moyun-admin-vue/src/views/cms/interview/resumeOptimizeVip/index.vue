    ...
    <el-dialog v-model="dialog.visible" :title="dialog.form.id ? '修改套餐' : '新增套餐'" width="520px" append-to-body>
      <el-form :model="dialog.form" label-width="90px">
        <el-form-item label="套餐名称" required>
          <el-input v-model="dialog.form.name" placeholder="如：月度会员" maxlength="32" />
        </el-form-item>
        ...
        <el-form-item label="权益说明">
          <el-input v-model="dialog.form.description" type="textarea" :rows="2" maxlength="255" placeholder="如：深度优化不限次、AI逐项建议、优化前后对比" />
        </el-form-item>
...
<script setup name="ResumeOptimizeVipPackage">
import { listResumeOptimizeVipPackage, addResumeOptimizeVipPackage, updateResumeOptimizeVipPackage, delResumeOptimizeVipPackage } from "@/api/cms/resumeOptimizeVip";
...
function loadList() {
  loading.value = true;
  listResumeOptimizeVipPackage({ name: queryName.value || undefined }).then(response => {
...
function openDialog(row) {
  dialog.form = row ? { ...row } : { name: "", price: 9.9, originalPrice: null, durationDays: 30, description: "", popular: false, sort: 0 };
  dialog.visible = true;
}
...
function saveDialog() {
  ...
  dialog.saving = true;
  const req = form.id ? updateResumeOptimizeVipPackage(form) : addResumeOptimizeVipPackage(form);
  req.then(() => {
...
function toggleStatus(row) {
  updateResumeOptimizeVipPackage({ id: row.id, status: row.status }).then(() => {
...
function handleDelete(row) {
  proxy.$modal.confirm('确认删除套餐「' + row.name + '」？（已有订阅订单的套餐仅可下架）').then(() => {
    return delResumeOptimizeVipPackage(row.id);
  }).then(() => {
