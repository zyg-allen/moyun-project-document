<template>
  <div class="app-container">
    ...
    <el-alert type="info" :closable="false" show-icon>
      <p>统计口径（前后台一致，金额单位：元，状态枚举 v11.79 统一）：GMV = 各渠道成功订单合计（App 打赏 + 门户打赏/付费阅读 + 记账VIP + 面试会员 + 简历优化会员，status=paid）；平台直接所得 = 公共通道 PLATFORM/credit 分录合计（分录为准不双算）；pay_order 为通道单据不重复计入 GMV（业务订单为准，避免双算）；守恒对账：理论公账余额 = 平台抽成累计 + Σ用户余额（提现已从余额扣除）。</p>
    </el-alert>
  </div>
</template>
