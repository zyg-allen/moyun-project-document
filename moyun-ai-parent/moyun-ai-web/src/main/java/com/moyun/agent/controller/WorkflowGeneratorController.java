...
import com.moyun.agent.service.WorkflowGeneratorService;
import com.moyun.agent.service.WorkflowGeneratorService.GenerateResult;
...
@Validated
@RestController
@RequestMapping("/api/workflow/generator")
...
public class WorkflowGeneratorController {
     ...
     * 根据自然语言描述生成工作流
     * 
     * <p>生成的工作流包含节点和边的完整定义，但不会自动保存到数据库</p>
     * 
     * @param request 生成请求，包含自然语言描述
     * @return 生成结果，包含工作流JSON定义和说明
     */
    @Operation(summary = "生成工作流", description = "根据自然语言描述生成工作流JSON，不自动保存")
    @PostMapping("/generate")
    public Result<GenerateResult> generate(@Valid @RequestBody GenerateRequest request) {
    ...
    public Result<Map<String, Object>> generateAndSave(@RequestBody GenerateRequest request) {
        log.info("🪄 收到工作流生成并保存请求: {}", request.getDescription());
        
        if (request.getDescription() == null || request.getDescription().trim().isEmpty()) {
            return Result.error("请输入工作流描述");
        }
        ...
        try {
            ...
            return Result.success("工作流已创建", Map.of(
    ...
    public Result<GenerateResult> optimize(
        ...
        try {
            ...
            if (result.isSuccess()) {
                log.info("✅ 工作流优化成功");
                return Result.success("优化成功", result);
            } else {
                return Result.error(result.getErrorMessage());
            }
        } catch (Exception e) {
            log.error("工作流优化失败", e);
            return Result.error("优化失败: " + e.getMessage());
        }
    }
    ...
    public static class GenerateRequest {
         ...
         * 自然语言描述
         * 
         * <p>详细描述想要生成的工作流流程，例如：</p>
         * <ul>
         *     <li>创建一个翻译流程：接收用户输入，翻译成英文后返回</li>
         *     <li>客服流程：判断问题类型，技术问题从知识库查询，其他直接AI回答</li>
         * </ul>
         */
    ...
    public static class OptimizeRequest {
         ...
         * <p>描述要如何修改现有工作流，例如：</p>
         * <ul>
         *     <li>在LLM节点之前添加一个条件判断</li>
         *     <li>将知识库检索节点替换为HTTP请求</li>
         * </ul>
         */
