        ...
        <div v-if="currentStatsAgent?.apiEnabled" class="api-info">
          ...
          <div class="api-docs">
            ...
            <div class="api-section">
              ...
              <code class="api-url">POST /api/agent/{{ currentStatsAgent.id }}/chat</code>
            </div>

            <div class="api-section">
              <div class="api-title">同步对话</div>
              <code class="api-url">POST /api/agent/{{ currentStatsAgent.id }}/chat/sync</code>
            </div>

            <div class="api-section">
              <div class="api-title">请求示例</div>
              <pre class="code-block">curl -X POST "{{ apiBaseUrl }}/api/agent/{{ currentStatsAgent.id }}/chat/sync" \
...
const loadKnowledgeLibraryList = async () => {
  try {
    const response = await axios.get('/api/knowledge-library/all')
    if (response.data.success) {
      knowledgeLibraryList.value = response.data.data || []
    }
  } catch (error) {
...
const loadModelConfigs = async () => {
  try {
    const response = await axios.get('/api/model-config/list')
    if (response.data.success) {
      // 只过滤出对话模型（chat类型）
      const allConfigs = response.data.data?.list || []
...
const loadAvailableDictionaries = async () => {
  try {
    const response = await axios.get('/api/agent/available-dictionaries')
    if (response.data.success) {
      availableDictionaries.value = response.data.data || []
    }
  } catch (error) {
...
const loadAvailableTools = async () => {
  try {
    const response = await axios.get('/api/tool/enabled')
    if (response.data.success) {
...
const editAgent = async (agent) => {
  ...
  // 加载智能体关联的词典
  try {
    const response = await axios.get(`/api/agent/${agent.id}/dictionaries`)
    if (response.data.success) {
      selectedDictionaries.value = response.data.data || []
    }
  } catch (error) {
  ...
  // 加载智能体关联的工具
  try {
    const response = await axios.get(`/api/tool/agent/${agent.id}/ids`)
    if (response.data.success) {
      selectedTools.value = response.data.data || []
    }
  } catch (error) {
...
const deleteAgent = async (id) => {
  try {
    ...
    const response = await axios.delete(`/api/agent/${id}`)
    if (response.data.success) {
...
const showAgentStats = async (agent) => {
  ...
  try {
    ...
    const [statsRes, sessionsRes] = await Promise.all([
      axios.get(`/api/agent/${agent.id}/stats`),
      axios.get(`/api/agent/${agent.id}/sessions?limit=10`)
    ])

    if (statsRes.data.success) {
      agentStats.value = {
        sessionCount: statsRes.data.data?.session_count || 0,
        messageCount: statsRes.data.data?.message_count || 0,
        totalTokens: statsRes.data.data?.total_tokens || 0,
        avgResponseTime: Math.round(statsRes.data.data?.avg_response_time || 0)
      }
    }

    if (sessionsRes.data.success) {
      agentSessions.value = sessionsRes.data.data || []
    }
  } catch (error) {
...
const duplicateAgent = async (agent) => {
  try {
    ...
    const response = await axios.post('/api/agent/create', newAgent)
    if (response.data.success) {
...
const clearAgentHistory = async () => {
  ...
  try {
    const response = await axios.delete(`/api/agent/${currentStatsAgent.value.id}/history`)
    if (response.data.success) {
...
const submitForm = async () => {
  ...
  try {
    const url = editMode.value ? '/api/agent/update' : '/api/agent/create'
    const method = editMode.value ? 'put' : 'post'
    const response = await axios[method](url, formData.value)

    if (response.data.success) {
      // 保存成功后，更新词典关联
      const agentId = editMode.value ? formData.value.id : response.data.data.id
      if (agentId) {
        try {
          await axios.post(`/api/agent/${agentId}/dictionaries`, selectedDictionaries.value)
        } catch (dictError) {
        ...
        // 更新工具关联
        try {
          await axios.post(`/api/tool/agent/${agentId}/bind`, selectedTools.value)
        } catch (toolError) {
...
const generateSystemPrompt = async () => {
  ...
  try {
    const response = await axios.post('/api/prompt/generate', {
      agentName: formData.value.name,
      description: inputText
    })

    if (response.data.success) {
      // 修复：后端返回的是 data.data.systemPrompt，不是 data.prompt
      formData.value.systemPrompt = response.data.data.systemPrompt
...
<style scoped src="@/styles/agent-manage.css"></style>
<style src="@/styles/agent-manage-global.css"></style>
