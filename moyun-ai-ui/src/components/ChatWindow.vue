...
window.submitFeedback = async function(button, feedbackType) {
  ...
  try {
    const response = await axios.post('/api/feedback/reference', {
...
const loadAgents = async () => {
  try {
    const response = await axios.get('/api/chat/agents')
    if (response.data.success) {
...
<img src='/api/image?path=${encodeURIComponent(source.imagePath)}' 
     style='max-width: 100%; border-radius: 6px; cursor: pointer; box-shadow: 0 2px 8px rgba(0,0,0,0.1);' 
     onclick="window.open('/api/image?path=${encodeURIComponent(source.imagePath)}', '_blank')" 
...
// 显示图片参考资料
const showImageReference = (detail) => {
  console.log('显示图片参考资料:', detail)
  
  const imageUrl = `/api/image?path=${encodeURIComponent(detail.imagePath)}`
    ...
    message: `
      <div style="text-align: center; padding: 20px;">
        ...
        <div style="position: relative; display: inline-block;">
          ...
          <div style="display: none; color: #f56c6c; padding: 40px; background: #fef0f0; border-radius: 8px; border: 1px solid #fbc4c4;">
            <i class="fa-solid fa-triangle-exclamation" style="font-size: 32px; margin-bottom: 10px;"></i>
            <div>图片加载失败</div>
            <div style="font-size: 12px; margin-top: 10px; color: #999;">路径：${detail.imagePath}</div>
          </div>
        </div>
...
const openOriginalFile = async (knowledgeBaseId, segmentIndex) => {
  ...
  try {
    const url = `/api/knowledge/${knowledgeBaseId}/file-info`
    ...
    if (infoResponse.data.success) {
      ...
      // 使用预览接口获取 PDF 文件
      const fileResponse = await axios.get(`/api/knowledge/${knowledgeBaseId}/preview`, {
        responseType: 'blob'
      })
...
const stopGeneration = async () => {
  ...
  try {
    ...
    const response = await axios.post(`/api/chat/abort/${currentConversationId.value}`)
    
    if (response.data.success && response.data.data) {
      console.log('✅ 已发送停止信号')
      ElMessage.info('已停止生成')
    } else {
      console.log('⚠️ 停止信号发送失败（会话可能已结束）')
    }
  } catch (error) {
  ...
  axios
    .post(
      ...
      { 
        ...
        message, 
        agentId: selectedAgentId.value,
        isGreeting: isGreeting,  // 告诉后端这是系统打招呼
        images: images  // 多模态图片（Base64数组）
      },
      {
        responseType: 'text',
        onDownloadProgress: (e) => {
          let fullText = e.event.target.responseText // 累积的完整文本
...
// 初始化 UUID
const initUUID = () => {
  let storedUUID = localStorage.getItem('user_base_uuid')
  if (!storedUUID) {
    storedUUID = uuidToNumber(uuidv4())
    localStorage.setItem('user_base_uuid', storedUUID)
  }
...
<style scoped src="@/styles/chat-window.css"></style>
