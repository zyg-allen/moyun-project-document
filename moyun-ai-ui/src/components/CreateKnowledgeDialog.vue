...
const nextStep = async () => {
  ...
  try {
    const response = await axios.get(`/api/knowledge/templates/recommended?fileType=${fileType}`)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const knowledgeId = uploadResponse.data.data.knowledgeId
    ...
    const configResponse = await axios.post('/api/knowledge/configure', configData)
    ...
    if (configResponse.data.success) {
      ...
      throw new Error(configResponse.data.message || '配置失败')
    }
  } catch (error) {
...
<style scoped src="@/styles/create-knowledge-dialog.css"></style>
