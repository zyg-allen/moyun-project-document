...
import QueryTemplates from './QueryTemplates.vue'
...
const loadDataSources = async () => {
  try {
    ...
    datasources.value = (res.data.data || []).filter(ds => ds.enabled)
...
const testConnection = async () => {
  ...
  try {
    const response = await axios.post('/api/data-analysis/datasource/test-connection', {
    ...
    if (response.data.success) {
      ...
      ElMessage.error(`连接失败：${response.data.message || '未知错误'}`)
    }
  } catch (error) {
    connectionStatus.value = 'failed'
...
const executeQuery = async () => {
  ...
  try {
    ...
    const res = await axios.post('/api/data-analysis/query', {
      ...
      signal: abortController.value.signal  // 支持取消请求
    })
    ...
    if (res.data.success) {
      ...
      console.log('查询结果:', {
        ...
        dataLength: res.data.data?.data?.length,
        columns: res.data.data?.columns?.length,
        sql: res.data.data?.generatedSql
      })
      ...
      if (res.data.data?.data?.length === 0) {
        ElMessage.warning(`查询成功，但未找到数据（共${res.data.data.totalCount}条）`)
      } else {
        ElMessage.success(`查询成功，返回${res.data.data?.totalCount || 0}条数据`)
...
const loadQueryHistory = async (datasourceId) => {
  try {
    const res = await axios.get(`/api/query-history/datasource/${datasourceId}`, {
      params: { limit: 5 }
    })
    if (res.data.success && res.data.data) {
      queryHistory.value = res.data.data.filter(h => h.status === 'success')
    }
  } catch (error) {
...
const generateDynamicSuggestions = async (datasourceId) => {
  try {
    const res = await axios.get(`/api/data-analysis/datasources/${datasourceId}/tables/info`)
