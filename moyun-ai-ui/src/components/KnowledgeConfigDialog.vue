...
const handleConfirm = async () => {
  ...
  try {
    ...
    const response = await axios.post('/api/knowledge/configure', configData)
    ...
    if (response.data.success) {
      ...
      ElMessage.error(response.data.message || '配置失败')
    }
  } catch (error) {
...
const startPollingStatus = (knowledgeId) => {
  const checkStatus = async () => {
    try {
      const response = await axios.get(`/api/knowledge/status/${knowledgeId}`)
...
<style scoped src="@/styles/knowledge-config-dialog.css"></style>
