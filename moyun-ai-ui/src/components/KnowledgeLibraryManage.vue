...
const loadLibraries = async () => {
  ...
  try {
    ...
    const res = await axios.get('/api/knowledge-library/list', { params })
    if (res.data.success) {
      libraries.value = res.data.data.records || []
      totalLibraries.value = res.data.data.total || 0
    }
  } catch (e) {
...
const createLibrary = async () => {
  ...
  creating.value = true
  try {
    const res = await axios.post('/api/knowledge-library', createForm.value)
    if (res.data.success) {
...
const openLibraryDetail = async (lib) => {
  ...
  try {
    const res = await axios.get(`/api/knowledge-library/${lib.id}`)
    if (res.data.success) {
      currentLibrary.value = res.data.data
...
const updateLibrary = async () => {
  ...
  updating.value = true
  try {
    const res = await axios.put(`/api/knowledge-library/${editForm.value.id}`, editForm.value)
    if (res.data.success) {
...
const deleteLibrary = async (lib) => {
  try {
    ...
    const res = await axios.delete(`/api/knowledge-library/${lib.id}`)
    if (res.data.success) {
...
const deleteDocument = async (doc) => {
  try {
    ...
    const res = await axios.delete(`/api/knowledge-library/${currentLibrary.value.id}/documents/${doc.id}`)
    if (res.data.success) {
      ElMessage.success('删除成功')
      openLibraryDetail(currentLibrary.value)
      loadLibraries()
...
const previewDocument = async (doc) => {
  ...
  try {
    // 使用预览接口获取 PDF 文件
    const response = await axios.get(`/api/knowledge/${doc.id}/preview`, {
      responseType: 'blob'
    })
...
const downloadFile = () => {
  if (currentPreviewDoc.value) {
    window.open(`/api/knowledge/${currentPreviewDoc.value.id}/download`, '_blank')
...
const openDocConfig = async (doc) => {
  try {
    // 获取推荐模板
    const response = await axios.get(`/api/knowledge/templates/recommended?fileType=${doc.fileType}`)
...
const startDocProcessing = async (doc) => {
  try {
    const response = await axios.post(`/api/knowledge/start-processing/${doc.id}`)
    if (response.data.success) {
...
const startDocPolling = (docId) => {
  ...
  const timer = setInterval(async () => {
    try {
      // 查询实时进度
      const progressResponse = await axios.get(`/api/knowledge/progress/${docId}`)
      ...
      // 查询文档状态
      const statusResponse = await axios.get(`/api/knowledge/${docId}`)
...
const viewDocDetail = async (doc) => {
  ...
  try {
    const response = await axios.get(`/api/knowledge/${doc.id}/segments`)
    if (response.data.success) {
      // API返回的是 data.list 而不是 data.data
      docSegments.value = response.data.data?.list || response.data.data || []
    }
  } catch (error) {
...
const executeRetrievalTest = async () => {
  ...
  try {
    const response = await axios.post(`/api/knowledge/${currentRetrievalDoc.value.id}/test-retrieval`, {
      query: retrievalTestForm.value.query,
      retrievalMode: retrievalTestForm.value.retrievalMode,
      topK: retrievalTestForm.value.topK
    })
...
const openLibraryRetrievalTest = async (lib) => {
  try {
    ...
    const res = await axios.get(`/api/knowledge-library/${lib.id}`)
    if (res.data.success) {
      currentRetrievalLibrary.value = res.data.data
    } else {
...
const executeLibraryRetrievalTest = async () => {
  ...
  try {
    const response = await axios.post(`/api/knowledge-library/${currentRetrievalLibrary.value.id}/test-retrieval`, {
      query: libraryRetrievalForm.value.query,
      retrievalMode: libraryRetrievalForm.value.retrievalMode,
      topK: libraryRetrievalForm.value.topK
    })
    
    if (response.data.success) {
      libraryRetrievalResults.value = response.data.data || []
...
const retryDocProcessing = async (doc) => {
  try {
    ...
    const response = await axios.post(`/api/knowledge/reprocess/${doc.id}`)
    if (response.data.success) {
