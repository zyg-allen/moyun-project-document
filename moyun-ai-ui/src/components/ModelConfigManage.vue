...
const submitForm = async () => {
  ...
  try {
    const url = editMode.value ? '/api/model-config/update' : '/api/model-config/create'
...
const deleteConfig = async (id) => {
  try {
    ...
    const response = await axios.delete(`/api/model-config/${id}`)
    if (response.data.success) {
...
const setDefault = async (id) => {
  try {
    const response = await axios.post(`/api/model-config/set-default/${id}`)
    if (response.data.success) {
...
const testConnection = async (config) => {
  try {
    ...
    const response = await axios.post('/api/model-config/test', config)
    if (response.data.success) {
...
<style scoped src="@/styles/model-config-manage.css"></style>
