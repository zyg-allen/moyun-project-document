...
const loadStats = async () => {
  ...
  try {
    ...
    const response = await axios.get('/api/token-usage/overview', { params })
...
const loadMetrics = async () => {
  try {
    const response = await axios.get('/api/token-usage/metrics')
    if (response.data.success) {
...
const flushLogs = async () => {
  flushing.value = true
  try {
    const response = await axios.post('/api/token-usage/flush')
    if (response.data.success) {
      ElMessage.success(`已同步 ${response.data.data.flushed} 条日志`)
      loadMetrics()
    } else {
...
<style scoped src="@/styles/token-usage-stats.css"></style>
