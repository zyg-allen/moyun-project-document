...
const toggleEnabled = async (tool) => {
  try {
    await axios.put(`/api/tool/${tool.id}/toggle?enabled=${tool.enabled}`)
...
const executeTest = async () => {
  ...
  try {
    const response = await axios.post(`/api/tool/test/${testForm.value.toolName}`, testForm.value.params)
    if (response.data.success) {
      testResult.value = response.data.data
    } else {
      testResult.value = { success: false, errorMessage: response.data.message }
    }
  } catch (error) {
...
<style scoped src="@/styles/tool-manage.css"></style>
