...
const handleGenerate = async () => {
  ...
  try {
    const response = await axios.post('/api/workflow/generator/generate', {
      description: description.value
    })
    ...
    const isSuccess = response.data.success === true ||
                      ...
                      (response.data.data && response.data.data.success)
    ...
    if (isSuccess) {
      ...
      result.value = {
        ...
        workflowName: data.workflowName || data.name || '生成的工作流',
        workflowDescription: data.workflowDescription || data.description || '',
      ...
      result.value = {
        success: false,
        errorMessage: response.data.message || response.data.msg || '生成失败'
      }
      ElMessage.error(result.value.errorMessage)
    }
  } catch (error) {
    ...
    ElMessage.error('生成失败: ' + result.value.errorMessage)
  } finally {
