 ...
 */
export function shouldShowCategory(cat) {
    if (!cat)
        return false;
 ...
 * @returns 过滤后的分类数组
 */
export function filterCategoryTree(categories) {
 ...
 */
export function getCategoryTarget(cat) {
    // linkType=1 表示外部链接
...
export const getCategoryTree = async (params) => {
    ...
    categoryTreeLoading = new Promise(async (resolve) => {
        try {
            const response = await httpGet('/portal/category/public/tree', params);
