...
export function useReadingProgress(bookId: Ref<string | number | undefined>) {
  ...
  const currentChapterNo = ref<number>(0);
   ...
   * 启动定时上报（章节加载完成后调用）
   */
  function startReporting(chapterId: string | number, chapterNo: number) {
    stopReporting();
   ...
   */
  async function restoreProgress(): Promise<number> {
    if (!isAuthenticated() || !bookId.value) return 0;
    try {
      const resp = await getReadingProgress(bookId.value);
      if (resp.code === 200 && resp.data) {
        const p = resp.data as ReadingProgress;
