...
export function searchArticles(query) {
    ...
    return mockArticles.filter(a => a.title.toLowerCase().includes(q) ||
...
export function addComment(articleId, author, content, parentId) {
    const comment = {
...
export function searchTags(keyword) {
    ...
    return mockTags.filter(tag => tag.name.toLowerCase().includes(kw) ||
...
// 从文章中提取标签建议（基于标题和分类）
export function getTagSuggestions(title, category) {
    const suggestions = [];
