...
// 轮播图 - 散文金句
const proseCarouselItems: CarouselItem[] = [
  {
    id: 'p1',
...
// 轮播图 - 技术热文
const techCarouselItems: CarouselItem[] = [
  {
    id: 't1',
    image: 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=800&h=500&fit=crop',
    title: 'Spring Boot 3.0 深度解析与实战',
    subtitle: '从原理到实践，全面掌握最新的 Spring Boot 技术栈。',
    author: '文 / 技术君',
    tag: '技术热点',
    articleId: '4',
  ...
  },
  {
    ...
    title: 'Vue3 + TypeScript 企业级应用开发',
    subtitle: '现代前端开发最佳实践，构建可维护的大型应用。',
    author: '文 / 前端达人',
    tag: '精选教程',
    articleId: '5',
  ...
  },
  {
    ...
    title: 'AI 编程实践：ChatGPT 提示工程指南',
    subtitle: '掌握 AI 辅助编程，提升开发效率 10 倍。',
    author: '文 / AI探索者',
    tag: 'AI前沿',
    articleId: '6',
    tags: ['AI', 'ChatGPT', '效率'],
...
// 轮播图控制
const nextProseSlide = () => {
  currentProseIndex.value = (currentProseIndex.value + 1) % proseCarouselItems.length
}

const prevProseSlide = () => {
  currentProseIndex.value = (currentProseIndex.value - 1 + proseCarouselItems.length) % proseCarouselItems.length
}

const nextTechSlide = () => {
  currentTechIndex.value = (currentTechIndex.value + 1) % techCarouselItems.length
}

const prevTechSlide = () => {
  currentTechIndex.value = (currentTechIndex.value - 1 + techCarouselItems.length) % techCarouselItems.length
}
  ...
  <div style="background-color: var(--theme-bg); min-height: 100vh;" class="flex flex-col">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div class="grid lg:grid-cols-2 gap-6">
        <!-- 左侧：散文金句轮播 -->
        <div class="relative h-[320px] sm:h-[360px] lg:h-[400px] overflow-hidden rounded-2xl group">
          <div class="absolute inset-0 bg-gradient-to-b from-transparent to-black/70 z-10"></div>
          <LazyImage
            :key="currentProseIndex"
            :src="proseCarouselItems[currentProseIndex].image"
            :alt="proseCarouselItems[currentProseIndex].title"
          ...
          <div class="absolute inset-0 z-20 flex flex-col justify-end p-6 sm:p-8">
            ...
            <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2 sm:mb-3 leading-tight">
              {{ proseCarouselItems[currentProseIndex].title }}
            </h2>
            <p class="text-sm sm:text-base text-gray-200 mb-4 sm:mb-6 line-clamp-2">
              {{ proseCarouselItems[currentProseIndex].subtitle }}
            </p>
            <div class="flex flex-wrap gap-2 mb-4 sm:mb-6">
              <span
                v-for="(tag, index) in proseCarouselItems[currentProseIndex].tags"
                :key="index"
            ...
            <div class="flex items-center justify-between">
              <span class="text-gray-300 text-sm">{{ proseCarouselItems[currentProseIndex].author }}</span>
              <button
                @click="router.push(`/article/${proseCarouselItems[currentProseIndex].articleId}`)"
                class="inline-flex items-center space-x-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-white text-red-600 rounded-full text-sm font-medium hover:bg-gray-100 transition-colors"
              >
                <span>阅读全文</span>
          ...
          <!-- 轮播控制按钮 -->
          <button
            @click="prevProseSlide"
            class="absolute left-3 top-1/2 -translate-y-1/2 z-30 w-8 h-8 sm:w-10 sm:h-10 bg-black/30 backdrop-blur text-white rounded-full flex items-center justify-center hover:bg-black/50 transition-all opacity-0 group-hover:opacity-100"
          >
            <ChevronLeft class="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <button
            @click="nextProseSlide"
            class="absolute right-3 top-1/2 -translate-y-1/2 z-30 w-8 h-8 sm:w-10 sm:h-10 bg-black/30 backdrop-blur text-white rounded-full flex items-center justify-center hover:bg-black/50 transition-all opacity-0 group-hover:opacity-100"
          >
            <ChevronRight class="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <!-- 指示器 -->
          <div class="absolute bottom-4 right-6 z-30 flex space-x-2">
            <button
              v-for="(_, index) in proseCarouselItems"
              :key="index"
              @click="currentProseIndex = index"
              :class="['w-2 h-2 rounded-full transition-all', currentProseIndex === index ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/70']"
            ></button>
          </div>
        </div>
        ...
        <div class="relative h-[320px] sm:h-[360px] lg:h-[400px] overflow-hidden rounded-2xl group">
          ...
          <LazyImage
            ...
            :src="techCarouselItems[currentTechIndex].image"
            :alt="techCarouselItems[currentTechIndex].title"
            class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
          />
          <div class="absolute inset-0 z-20 flex flex-col justify-end p-6 sm:p-8">
            <div class="inline-flex items-center space-x-2 bg-gradient-to-r from-blue-600 to-indigo-500 text-white px-3 py-1 rounded-full text-xs font-medium mb-3 sm:mb-4 w-fit">
              <Code class="w-3 h-3" />
              <span>{{ techCarouselItems[currentTechIndex].tag }}</span>
            </div>
            <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold text-white mb-2 sm:mb-3 leading-tight">
              {{ techCarouselItems[currentTechIndex].title }}
            </h2>
            <p class="text-sm sm:text-base text-gray-200 mb-4 sm:mb-6 line-clamp-2">
              {{ techCarouselItems[currentTechIndex].subtitle }}
            </p>
            <div class="flex flex-wrap gap-2 mb-4 sm:mb-6">
              <span
                v-for="(tag, index) in techCarouselItems[currentTechIndex].tags"
                :key="index"
                class="px-2.5 py-1 text-xs rounded-full bg-white/20 backdrop-blur text-white"
              >
                {{ tag }}
            ...
            <div class="flex items-center justify-between">
              <span class="text-gray-300 text-sm">{{ techCarouselItems[currentTechIndex].author }}</span>
              <button
                @click="router.push(`/article/${techCarouselItems[currentTechIndex].articleId}`)"
                class="inline-flex items-center space-x-2 px-4 sm:px-5 py-2 sm:py-2.5 bg-white text-blue-600 rounded-full text-sm font-medium hover:bg-gray-100 transition-colors"
              >
                <span>阅读全文</span>
                <ChevronRight class="w-4 h-4" />
              </button>
          ...
          <div class="absolute bottom-4 right-6 z-30 flex space-x-2">
            <button
              v-for="(_, index) in techCarouselItems"
              :key="index"
              @click="currentTechIndex = index"
              :class="['w-2 h-2 rounded-full transition-all', currentTechIndex === index ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/70']"
            ></button>
          </div>
        </div>
      </div>
    ...
    <!-- 2. 名言提示和写作入口 -->
    <div class="bg-gradient-to-r from-amber-50 to-orange-50 border-y border-amber-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-0">
          ...
          >
            ...
            <div class="w-8 h-8 sm:w-10 sm:h-10 bg-white/20 rounded-full flex items-center justify-center flex-shrink-0 ml-4">
    ...
    <!-- 3. 内容精选区 -->
    <div class="py-6 sm:py-8 lg:py-10" style="background-color: var(--theme-bg);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div>
            <div class="flex items-center justify-between mb-4 sm:mb-6">
              <div class="flex items-center gap-2 sm:gap-3">
                <div class="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-red-100 to-orange-100 rounded-xl flex items-center justify-center">
              ...
              <button @click="viewMore('散文天地')" class="flex items-center gap-1.5 text-xs sm:text-sm font-medium" style="color: var(--theme-text-secondary);">
                <span>更多</span>
            ...
            <div class="space-y-3 sm:space-y-4">
              ...
              >
                <div v-if="index < 2" class="relative w-28 h-20 sm:w-36 sm:h-24 flex-shrink-0 rounded-lg overflow-hidden">
                  ...
                  />
                  <div v-if="index === 0" class="absolute top-1.5 left-1.5 px-2 py-0.5 bg-red-600 text-white text-xs rounded">置顶</div>
                </div>
          ...
          <div class="lg:space-y-6">
            ...
            <div class="p-4 sm:p-5 rounded-2xl" style="background-color: var(--theme-surface);">
              <div class="flex items-center gap-2 sm:gap-3 mb-4">
                <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-xl flex items-center justify-center">
              ...
              <div class="space-y-3">
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <h4 class="font-medium text-xs sm:text-sm line-clamp-2 text-left mb-1" style="color: var(--theme-text);">
                      {{ article.title }}
                    </h4>
                    <div class="flex items-center gap-3">
                      <span class="flex items-center gap-1 text-xs" style="color: var(--theme-text-secondary);">
    ...
    <!-- 4. 专题推荐区 -->
    <div class="py-6 sm:py-8 lg:py-10 border-t" style="background-color: var(--theme-bg); border-color: var(--theme-border);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <!-- 读书空间专题 -->
        <div class="mb-8 sm:mb-10">
          <div class="flex items-center justify-between mb-4 sm:mb-5">
            <div class="flex items-center gap-2 sm:gap-3">
              <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-emerald-100 to-teal-100 rounded-xl flex items-center justify-center">
            ...
            <button @click="viewMore('读书空间')" class="flex items-center gap-1.5 text-xs sm:text-sm font-medium" style="color: var(--theme-text-secondary);">
              <span>进入读书空间</span>
              <ArrowRight class="w-3.5 h-3.5" />
            </button>
          </div>
          <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            <div class="p-4 sm:p-5 rounded-xl relative overflow-hidden" style="background: linear-gradient(135deg, #134e5e 0%, #71b280 100%);">
              ...
              <div class="relative">
                ...
                <button class="px-4 py-2 bg-white text-emerald-700 rounded-full text-sm font-medium hover:bg-emerald-50 transition-colors">
                  立即加入
                </button>
              </div>
            </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-4 sm:mb-5">
            <div class="flex items-center gap-2 sm:gap-3">
              <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-purple-100 to-pink-100 rounded-xl flex items-center justify-center">
            ...
            <button @click="viewMore('面试指南')" class="flex items-center gap-1.5 text-xs sm:text-sm font-medium" style="color: var(--theme-text-secondary);">
              <span>进入面试指南</span>
              <ArrowRight class="w-3.5 h-3.5" />
            </button>
          </div>
    ...
    <div class="py-6 sm:py-8 lg:py-10 border-t" style="background-color: var(--theme-bg); border-color: var(--theme-border);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-4 sm:mb-5">
          <div class="flex items-center gap-2 sm:gap-3">
            <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl flex items-center justify-center">
        ...
        <div class="p-4 sm:p-5 rounded-2xl" style="background-color: var(--theme-surface);">
          <div class="flex items-center justify-between mb-4">
            ...
            <button @click="viewMore(activeTheme)" class="flex items-center gap-1.5 text-xs sm:text-sm" style="color: var(--theme-text-secondary);">
              <span>查看更多</span>
              <ArrowRight class="w-3.5 h-3.5" />
            </button>
          </div>
          <div class="grid sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            >
              ...
              <span class="text-xs flex-shrink-0" style="color: var(--theme-text-secondary);">{{ article.createdAt || '2024-01-15' }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="py-6 sm:py-8 lg:py-10" style="background-color: var(--theme-bg);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-4 sm:mb-5">
          <div class="flex items-center gap-2 sm:gap-3">
            <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-cyan-100 to-blue-100 rounded-xl flex items-center justify-center">
          ...
          <button @click="router.push('/authors')" class="flex items-center gap-1.5 text-xs sm:text-sm" style="color: var(--theme-text-secondary);">
            <span>全部作者</span>
            <ArrowRight class="w-3.5 h-3.5" />
          </button>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3 sm:gap-4">
          <div
            ...
            style="background-color: var(--theme-surface);"
          >
            <div class="w-11 h-11 sm:w-12 sm:h-12 mx-auto mb-3 rounded-full bg-gradient-to-br from-red-100 to-orange-100 flex items-center justify-center">
              <span class="text-base sm:text-lg font-bold" style="color: var(--theme-primary);">{{ author.avatar }}</span>
            </div>
            ...
            <button class="mt-3 px-3 py-1.5 rounded-full text-xs border transition-colors hover:bg-gray-50" style="border-color: var(--theme-primary); color: var(--theme-primary);">
              关注作者
            </button>
          </div>
        </div>
    ...
    <!-- 7. 底部互动区 -->
    <div class="py-6 sm:py-8 lg:py-10 border-t" style="background-color: var(--theme-surface); border-color: var(--theme-border);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid lg:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div class="p-4 sm:p-5 rounded-2xl" style="background-color: var(--theme-bg);">
            ...
            <div class="space-y-3">
              <div class="p-3 sm:p-4 rounded-xl bg-gradient-to-r from-rose-50 to-pink-50 cursor-pointer hover:from-rose-100 hover:to-pink-100 transition-colors">
                ...
                <p class="text-xs mb-3" style="color: var(--theme-text-secondary);">分享关于美食的记忆，讲述背后的故事</p>
                <div class="flex items-center justify-between">
              ...
              <div class="p-3 sm:p-4 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors" style="background-color: var(--theme-surface);">
          ...
          <div class="p-4 sm:p-5 rounded-2xl" style="background-color: var(--theme-bg);">
            <div class="flex items-center gap-2 sm:gap-3 mb-4">
              <div class="w-10 h-10 sm:w-11 sm:h-11 bg-gradient-to-br from-green-100 to-emerald-100 rounded-xl flex items-center justify-center">
            ...
            <div class="space-y-3">
              <div class="p-3 sm:p-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50">
                ...
                <div class="space-y-2">
                  <div class="flex items-center gap-3">
                    <span class="w-6 h-6 rounded-full bg-gradient-to-br from-amber-400 to-yellow-500 text-white text-xs flex items-center justify-center font-bold">1</span>
                  ...
                  <div class="flex items-center gap-3">
                    <span class="w-6 h-6 rounded-full bg-gradient-to-br from-gray-300 to-gray-400 text-white text-xs flex items-center justify-center font-bold">2</span>
                  ...
                  <div class="flex items-center gap-3">
                    <span class="w-6 h-6 rounded-full bg-gradient-to-br from-amber-600 to-orange-500 text-white text-xs flex items-center justify-center font-bold">3</span>
    ...
    <div class="py-6 sm:py-8 lg:py-10 border-t" style="background-color: var(--theme-bg); border-color: var(--theme-border);">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="space-y-6 sm:space-y-8">
          <!-- 热门标签 -->
          <div class="rounded-2xl p-4 sm:p-5" style="background-color: var(--theme-surface); border: 1px solid var(--theme-border);">
