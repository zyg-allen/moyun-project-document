<template>
  <div class="min-h-screen bg-theme-bg">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
      <!-- 页头 -->
      <div class="mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-theme-text mb-1">面试会员</h1>
        <p class="meta-text text-theme-text-secondary">
          解锁语音面试不限场次、深度面试报告与题库全量访问，订阅即刻生效、续费顺延不折损。
        </p>
      </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="meta-text" :class="vip.isVip ? 'text-slate-300' : 'text-theme-text-secondary'">
              {{ vip.isVip ? '权益有效期至 ' + formatDay(vip.vipExpire) : '开通后解锁全部面试高级功能' }}
            </p>
          </div>
        </div>
      </div>
...
import {
  getInterviewVipPackages,
  getInterviewVipStatus,
  subscribeInterviewVip,
  type InterviewVipPackageVO,
} from '@/api/interviewVip';
...
const benefits = ['语音面试不限场次（非会员可免费体验2次）', '深度面试报告', '题库全量访问', '简历深度优化折扣'];

const loading = ref(false);
const packages = ref<InterviewVipPackageVO[]>([]);
...
onMounted(async () => {
  ...
  try {
    const [pkgRes, statusRes] = await Promise.all([
      getInterviewVipPackages(),
      getInterviewVipStatus().catch(() => null),
    ]);
...
function genClientUuid() {
  return 'ivip-' + Date.now() + '-' + Math.random().toString(36).substring(2, 10);
}
...
async function doSubscribe() {
  ...
  try {
    const res = await subscribeInterviewVip({
      packageId: selectedId.value,
      clientUuid: genClientUuid(),
    });
    const d = res.data;
    if (d?.payNo) {
      // 跳通用收银台（v11.82 公共通道，与打赏/付费阅读同构）
      router.push({
        path: '/pay/cashier',
        query: {
          payNo: d.payNo,
