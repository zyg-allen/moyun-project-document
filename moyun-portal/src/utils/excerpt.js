...
class LocalExcerptStrategy {
     ...
     * 提取完整的句子
     */
    extractCompleteSentences(text, maxLength = 200) {
        if (!text)
            return '';
    ...
    truncateAtWord(text, maxLength) {
        ...
        const lastPunctuation = Math.max(truncated.lastIndexOf('，'), truncated.lastIndexOf(','), truncated.lastIndexOf('。'), truncated.lastIndexOf('！'), truncated.lastIndexOf('？'), truncated.lastIndexOf('!'), truncated.lastIndexOf('?'));
    ...
    async extract(content, editorMode = 'richtext', maxLength = 200) {
        if (!content)
            return '';
 ...
 * 快速提取摘要（向后兼容）
 */
export async function extractExcerpt(content, editorMode = 'richtext', maxLength = 200) {
