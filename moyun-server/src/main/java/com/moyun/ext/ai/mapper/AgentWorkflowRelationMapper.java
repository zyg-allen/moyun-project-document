...
public interface AgentWorkflowRelationMapper extends BaseMapper<AgentWorkflowRelation> {
     ...
     * 获取智能体绑定的工作流ID列表
     */
    @Select("SELECT workflow_id FROM ai_agent_workflow_relation WHERE agent_id = #{agentId} AND enabled = 1 ORDER BY sort_order")
    List<Long> selectWorkflowIdsByAgentId(@Param("agentId") Long agentId);
    
    /**
     * 获取绑定了该工作流的智能体ID列表
     */
    @Select("SELECT agent_id FROM ai_agent_workflow_relation WHERE workflow_id = #{workflowId} AND enabled = 1")
