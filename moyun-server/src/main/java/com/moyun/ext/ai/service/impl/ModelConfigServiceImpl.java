...
public class ModelConfigServiceImpl extends ServiceImpl<ModelConfigMapper, ModelConfig> implements ModelConfigService {
     ...
     */
    private final Map<String, ChatLanguageModel> chatModelClientCache = new ConcurrentHashMap<>();
    private final Map<String, StreamingChatLanguageModel> streamingModelClientCache = new ConcurrentHashMap<>();
    ...
    public ChatLanguageModel createChatModel(Long configId, Double temperature, Integer maxTokens) {
        return createChatModel(configId, temperature, maxTokens, false);
    }
    ...
    public ChatLanguageModel createChatModel(Long configId, Double temperature, Integer maxTokens, boolean jsonMode) {
        ...
        if (temperature != null || maxTokens != null) {
            ...
            overriddenConfig.setSupportsJsonMode(config.getSupportsJsonMode());
            ...
            ChatLanguageModel model = createChatModelFromConfig(overriddenConfig, effectiveJsonMode);
            chatModelClientCache.put(clientKey, model);
            return model;
        }

        ChatLanguageModel model = createChatModelFromConfig(config, effectiveJsonMode);
        chatModelClientCache.put(clientKey, model);
        return model;
    }
    ...
    public StreamingChatLanguageModel createStreamingChatModel(Long configId, Double temperature, Integer maxTokens) {
        ...
        if (temperature != null || maxTokens != null) {
            ...
            StreamingChatLanguageModel model = createStreamingChatModelFromConfig(overriddenConfig);
            streamingModelClientCache.put(clientKey, model);
            return model;
        }

        StreamingChatLanguageModel model = createStreamingChatModelFromConfig(config);
        streamingModelClientCache.put(clientKey, model);
        return model;
    }
    ...
    private ChatLanguageModel createChatModelFromConfig(ModelConfig config, boolean jsonMode) {
        ...
        switch (apiStyle) {
            case AiProvider.STYLE_OLLAMA_NATIVE:
                return createOllamaChatModel(config, jsonMode);
            case AiProvider.STYLE_OPENAI_COMPATIBLE:
            default:
                return createOpenAiCompatibleChatModel(config, jsonMode);
        }
    }
     ...
     */
    private ChatLanguageModel createOpenAiCompatibleChatModel(ModelConfig config, boolean jsonMode) {
