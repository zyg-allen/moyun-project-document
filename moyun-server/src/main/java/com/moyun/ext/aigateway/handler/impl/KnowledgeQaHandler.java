...
import com.moyun.ext.aigateway.model.data.KnowledgeQaSceneData;
import com.moyun.ext.aigateway.model.data.KnowledgeQaSceneData.Reference;
...
public class KnowledgeQaHandler extends AbstractAiSceneHandler {
    ...
    public String getSceneCode() {
        return AiSceneEnum.KNOWLEDGE_QA.getCode();
    }
    ...
    public AiExecuteResponse<?> execute(AiExecuteRequest request, AiSceneConfig config) {
        ...
        try {
            ...
            if (answer.isEmpty()) {
                log.warn("[aigateway:{}] LLM 未返回有效回答（模型不可用或空内容）", getSceneCode());
    ...
    private Agent loadAgent(AiSceneConfig config) {
        ...
        try {
            ...
            log.warn("[aigateway:{}] Agent 加载失败（按默认 RAG 配置执行）: {}", getSceneCode(), e.getMessage());
        }
        return new Agent();
    }
    ...
    private List<Long> parseKbIds(Object raw) {
        ...
        if (text.startsWith("[")) {
            try {
                ...
                log.warn("[aigateway:{}] 知识库ID JSON解析失败: {}", getSceneCode(), text);
                return result;
            }
        }
    ...
    private void addKbId(List<Long> result, String value) {
        ...
        try {
            ...
            log.warn("[aigateway:{}] 非法知识库ID已忽略: {}", getSceneCode(), trimmed);
        }
    }
