...
/**
 * 记账VIP订单（接入公共支付通道，bizType=ledger_vip 平台直收类）
 *
 * <p>链路：选套餐下单（快照 name/duration，clientUuid 幂等）→ 落 pending 单 →
 * payGateway 统一下单（platform=ledger_app）→ 收银台（扫码/mock）→ 回调置 paid +
 * 权益顺延（vip_expire 从 max(now, 现有到期) 顺延 duration_days）+ settlePlatform
 * 平台全额分账。
 *
 * <p>状态枚举 统一：pending/paid/refunded/closed。
 *
 * @author moyun
 */
@Data
@TableName("ledger_vip_order")
public class LedgerVipOrder {
