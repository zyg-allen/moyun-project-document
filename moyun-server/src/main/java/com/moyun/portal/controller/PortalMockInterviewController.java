 ...
 * @author moyun
 */
@Tag(name = "AI 模拟面试官", description = "模拟面试开始、作答、评分、结束、历史")
@RestController
...
public class PortalMockInterviewController extends BaseController {
    ...
    @Operation(summary = "开始模拟面试", description = "按岗位/场景从题库抽取 5 道题，生成面试会话")
    @PostMapping("/start")
    public AjaxResult start(@RequestBody StartRequest body) {
        ...
        return AjaxResult.success(mockInterviewService.start(userId, position, scene));
    }

    @Operation(summary = "面试详情", description = "查询面试会话详情（含问答列表与已答数）")
    @GetMapping("/{id}")
    ...
    @Operation(summary = "结束面试", description = "结束面试并生成总结（自动计算平均分）")
